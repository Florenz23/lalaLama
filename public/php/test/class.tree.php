...
class tree {
	...
	public function mk($parent, $position = 0, $data = array()) {
		...
		// rename node and insert other infos
		if ($data && count($data)) {
...
// übergeben wird 3 Parameter aus mk beim erstellen eines nodes
	...
	public function get_node($id, $options = array()) {
		$node = $this->db->one("
			...
			WHERE
				s." . $this->options['structure']['user_id'] . " = " . $this->user_id . " AND
	...
	public function mv($id, $parent, $position = 0) {
		...
		$sql[] = "
			UPDATE " . $this->options['structure_table'] . "
				...
				WHERE " . $this->options['structure']["user_id"] . " = " . $this->user_id . " AND " .
		$this->options['structure']["id"] . " IN(" . implode(',', $tmp) . ")
		";
		...
		" WHERE " . $this->options['structure']["user_id"] . " = " . $this->user_id . " AND " .
		$this->options['structure']["id"] . "  = " . (int) $id[$this->options['structure']['id']] . "
		";
	...
	public function rm($id) {
		...
		" AND " . $this->options['structure']['user_id'] . " = " . $this->user_id .
		...
		if ($this->options['data_table']) {
			...
			$sql[] = "DELETE FROM " . $this->options['data_table'] .
			" WHERE " . $this->options['data2structure'] .
			" IN (" . implode(',', $tmp) . ")";
		}
